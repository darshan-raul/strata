...
- `accio/` — main application workspace (Go monorepo + portal-ui)
...
Each service has its own `go.mod`. Dependencies are NOT hoisted to `accio/`.
...
golangci-lint run --path-prefix=accio/services/catalog-service --timeout=5m

# Test a single service
cd accio/services/catalog-service && go test -v ./...

# Run portal-ui linter
cd accio/services/portal-ui && npm run lint
```