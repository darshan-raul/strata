# Strata Kubernetes Setup for Kind v1.36

This guide walks through deploying the Strata application suite on a Kind (Kubernetes in Docker) cluster v1.36.
...
```bash
kind create cluster --config Strata-kind.yaml --image kindest/node:v1.36.0
```
...
docker run -d --restart=always -p 5000:5000 --name Strata-registry registry:2
...
```bash
kind load docker-image auth-service:latest --name Strata
kind load docker-image catalog-service:latest --name Strata
kind load docker-image provisioner-service:latest --name Strata
kind load docker-image scorecard-service:latest --name Strata
kind load docker-image workflow-engine:latest --name Strata
kind load docker-image audit-service:latest --name Strata
kind load docker-image portal-ui:latest --name Strata
```
...
kubectl wait --for=condition=available --timeout=120s deployment/postgres -n Strata
...
|----------------|----------------------------------------|-----------|
| Portal UI      | http://Strata.localhost:3000            | 30000     |
...
```bash
# Auth Service
kubectl port-forward -n Strata svc/auth-service 8086:8086

# Catalog Service
kubectl port-forward -n Strata svc/catalog-service 8081:8081

# Provisioner Service
kubectl port-forward -n Strata svc/provisioner-service 8082:8082

# Scorecard Service
kubectl port-forward -n Strata svc/scorecard-service 8083:8083

# Workflow Engine
kubectl port-forward -n Strata svc/workflow-engine 8084:8084

# Audit Service
kubectl port-forward -n Strata svc/audit-service 8085:8085
```
...
kubectl delete namespace Strata
```
...
```bash
kind get nodes -n Strata
docker exec -it Strata-control-plane crictl images
```

### PostgreSQL Not Starting
Check init logs:
```bash
kubectl logs -n Strata deployment/postgres
```
